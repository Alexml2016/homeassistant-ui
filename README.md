...
│   └── garage/      # панель управления воротами
├── shared/
...
Панель управления откатными дворовыми и подъёмными гаражными воротами находится
в `panels/garage`.
...
Панель определяет состояние каждых ворот по датчикам открытого и закрытого
положения, отображает движение, температуру и активность микроволнового датчика.
Нажатие на карточку ворот вызывает `cover.toggle`.
